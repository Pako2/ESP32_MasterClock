...
#    download_speed = join(str(board.get("download.speed", "115200")))
    ...
    if "none" in upload_port:
        env.AutodetectUploadPort()
        upload_port = join(env.get("UPLOAD_PORT", "none"))
    fs_file = join(env.subst("$BUILD_DIR"), f"unpacked_fs_{hex(fs_info.start)}_{hex(fs_info.length)}.bin")
    esptoolpy_flags = [
            "--chip", mcu,
            "--port", upload_port,
            "--baud",  download_speed,
...
def build_webh_files(*args, **kwargs):
    ...
    with open(unpackeddir+"/masterclock.js","rb") as f:
        ordinary = f.read()
    ...
    res=bytes("#define masterclock_js_gz_len {}\nconst uint8_t masterclock_js_gz[] PROGMEM = {{\n".format(lngth), 'utf8')
    j=0
    ...
    res = res[:-1]
    res+=bytes("\n};", 'utf8')
    with open('include/webh/masterclock.js.gz.h', 'wb') as wb_writer:
        wb_writer.write(res)
    ...
    with open(unpackeddir+"/masterclock.html","rb") as f:
        html = f.read()
    minified = htmlmin.minify(html.decode("utf-8"), remove_empty_space=True)
    out = BytesIO()
    with gzip.GzipFile(fileobj=out, mode="w") as f:
    ...
    content=out.getbuffer()
    lngth = content.nbytes
    res=bytes("#define masterclock_html_gz_len {}\nconst uint8_t masterclock_html_gz[] PROGMEM = {{\n".format(lngth), 'utf8')
    j=0
    for i in range(lngth):
        res+=bytes("0x{:02x},".format(content[i]), 'utf8')
        j+=1
    ...
    res = res[:-1]
    res+=bytes("\n};", 'utf8')
    with open('include/webh/masterclock.html.gz.h', 'wb') as wb_writer:
        wb_writer.write(res)
